...
The browser obtains or resumes an anonymous session through `POST /api/v1/session`. The `ratapp_session` HttpOnly cookie is the only browser-facing session identity; resource requests use same-origin credentials and never include a session ID in their URLs or payloads. The cookie and environment-scoped Redis keys share the configured 48-hour TTL. The Phase 5 prompt route intentionally returns `501 classification_unavailable`; Phase 6 will replace that endpoint behavior.
