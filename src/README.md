...
- `server/session/session-handler.ts` provides the route wrappers: optional sessions for reads and lazy session creation for stateful operations.
...
- `DashboardPage` owns the page-wide one-at-a-time mutation gate and refresh signal. Reads can render empty state without a session; the first stateful mutation creates one transparently.
...
The browser starts without creating an anonymous session. Stateless reads return empty state when no valid session cookie exists, and the first stateful mutation creates the Redis session and issues the `ratapp_session` HttpOnly cookie transparently. The cookie is the only browser-facing session identity; resource requests use same-origin credentials and never include a session ID in their URLs or payloads. The cookie and environment-scoped Redis keys share the configured 48-hour TTL, and resolving an existing session does not refresh it. `POST /api/v1/expenses/prompt` sends only the exact prompt, locale, and current real-category names to the server-only OpenAI extractor. The server validates candidates, assigns IDs and timestamps, persists accepted expenses, and returns safe error envelopes. Configure `OPENAI_API_KEY` and `OPENAI_MODEL` only in the server environment; `OPENAI_MODEL=gpt-4.1-nano` selects the classification model.

For a manual development test, first create a category or submit an expense prompt, inspect the development-only `ratapp_session` cookie value in browser developer tools, then run `npm run seed:redis -- --session-id <uuid>`. The API itself never returns the ID. The command loads `.env.development.local`, verifies that the session already exists, writes trusted deterministic category and expense fixtures directly with the concrete Redis client, and renews the configured TTL. It never creates or registers a session, and production seeding is refused.
