...
- **R6 — Localization:** Keep all visible and accessible copy in synchronized English (`en`) and Spanish (`es`) dictionaries. Support immediate in-memory switching through an accessible `EN`/`ES` control.
- **R7 — Amounts and chart:** Keep amounts as integer minor units and display `$` with two locale-aware decimal places. Derive the chart, legend, category values, and total from the same fixture data.
